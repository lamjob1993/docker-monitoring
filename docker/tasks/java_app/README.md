...
Если ты создаёшь сайт на Java, который должен обрабатывать запросы по URL (например, `/login`, `/user/123`), то тебе нужен сервер, чтобы этот код запустить.  
Tomcat — один из самых популярных серверов для этой задачи.

---

#### 🧰 Как работает?

1. Ты пишешь Java-приложение (например, используя Servlet'ы).
2. Собираешь его в файл `.war` (Web ARchive).
3. Кладёшь этот файл в папку `webapps` Tomcat.
4. Запускаешь Tomcat.
5. Сервер запускает твоё приложение и начинает обрабатывать запросы.
...
🔹 **Tomcat** — это сервер для запуска Java-веб-приложений.  
🔹 Он поддерживает **Servlet'ы**, **JSP**, **REST API**.  
🔹 Популярен среди Java-разработчиков за простоту установки и использования.
...
#### 🟢 Почему используют Servlet'ы?

- ✅ Позволяют писать динамический контент.
- ✅ Интегрируются с JSP, JSF, Spring и другими фреймворками.
- ✅ Работают в любом совместимом веб-контейнере (Tomcat, Jetty, WildFly и т.д.).

---

#### 🔹 Коротко:

🔹 **Servlet** — это Java-класс, который обрабатывает HTTP-запросы и отвечает на них.  
🔹 Он используется для создания веб-приложений на Java.  
🔹 Servlet — основа более сложных технологий, таких как **Spring MVC** и **Jakarta EE**.
